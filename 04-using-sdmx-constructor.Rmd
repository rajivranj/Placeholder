...
In the context of SDMX, a Concept Scheme is a structured list of concepts used to define the dimensions and attributes of a dataset. When creating a data model, it's important to identify and define the key concepts that are relevant to the data being collected.
...
To create Code Lists and Concept Schemes, you can use the SDMX Constructor by following these steps. First, upload the Code List and then the Concept Scheme. By completing these steps in sequence, you can create both artefacts in one go.
...
Recalling our two initial tables (Table: \@ref(tab:table41) and Table: \@ref(tab:table42)), we will now generate DSDs, ContentConstraints and Dataflows. We will also create a CategoryScheme. We can create all these artefacts through the [Table Modeller](#table-modeller) option in the SDMX Constructor.
